...
\begin{itemize}
    ...
    \item \textbf{Quản lý và giám sát tập trung:} Cung cấp một giao diện quản trị cho phép người quản trị theo dõi trạng thái hoạt động của các luồng dữ liệu, quản lý người dùng, phân quyền và cấu hình các tác vụ tích hợp.
...
\begin{itemize}
    \item \textbf{Tính bảo mật:} Dữ liệu, đặc biệt là thông tin cá nhân của sinh viên và giảng viên, phải được bảo vệ nghiêm ngặt ở cả trạng thái tĩnh (at-rest) và trạng thái động (in-transit). Hệ thống phải có cơ chế chống lại các hình thức tấn công phổ biến.
    \item \textbf{Tính mở rộng:} Kiến trúc phải cho phép mở rộng dễ dàng khi có thêm các hệ thống nguồn mới hoặc khi lưu lượng truy cập từ các ứng dụng người dùng cuối tăng cao. Các thành phần phải có khả năng mở rộng độc lập.
    \item \textbf{Tính tái sử dụng:} Các thành phần tích hợp, đặc biệt là các API, phải được thiết kế để có thể tái sử dụng cho nhiều mục đích và ứng dụng khác nhau, nhằm giảm thiểu thời gian và chi phí phát triển trong tương lai.
    \item \textbf{Tính sẵn sàng:} Hệ thống phải hoạt động ổn định và có độ sẵn sàng cao để không làm gián đoạn hoạt động của các ứng dụng phụ thuộc vào nó.
    \item \textbf{Khả năng bảo trì:} Kiến trúc phải được phân tách rõ ràng, các thành phần có trách nhiệm duy nhất, giúp cho việc bảo trì, sửa lỗi và nâng cấp trở nên dễ dàng hơn.
\end{itemize}
...
\subsection{Các hệ thống nguồn dữ liệu}
...
\subsection{Trục tích hợp thông tin}
...
Thành phần này chịu trách nhiệm cho luồng dữ liệu đầu vào, hiện thực hóa lớp \textbf{System API}:
\begin{itemize}
    ...
    \item Sau khi trích xuất, máy chủ này sẽ thực hiện các bước biến đổi dữ liệu cơ bản (ETL/ELT) và nạp dữ liệu đã được làm sạch, chuẩn hóa vào Kho dữ liệu trung tâm.
\end{itemize}

\subsubsection{Máy chủ Chia sẻ Dữ liệu (Data Sharing Server)}
Thành phần này chịu trách nhiệm cho luồng dữ liệu đầu ra, hiện thực hóa hai lớp \textbf{Process API} và \textbf{Experience API}:
\begin{itemize}
    \item \textbf{Process API:} Lớp này chứa các logic nghiệp vụ phức tạp. Nó thực hiện truy vấn đến Kho dữ liệu trung tâm, kết hợp và tổng hợp thông tin để tạo ra các thực thể dữ liệu có giá trị nghiệp vụ cao.
...
\begin{itemize}
    \item \textbf{Xác thực:} Kiểm tra danh tính của người dùng/ứng dụng gọi đến.
    \item \textbf{Định tuyến:} Chuyển tiếp yêu cầu hợp lệ đến Experience API tương ứng trên Máy chủ Chia sẻ Dữ liệu.
    \item \textbf{Kiểm soát:} Áp dụng các chính sách bảo mật, giới hạn tần suất truy cập để bảo vệ hệ thống.
\end{itemize}
\begin{figure}
    \centering
...
\begin{enumerate}
    \item \textbf{Yêu cầu đăng nhập:} Người dùng gửi thông tin định danh (ví dụ: username và password) đến ứng dụng backend.
    \item \textbf{Xác minh thông tin:} Ứng dụng backend truy vấn vào CSDL, cụ thể là bảng \texttt{Users}, để kiểm tra tính hợp lệ của thông tin đăng nhập.
...
\subsubsection{Luồng phân quyền}
